...
/// The accessible role that an object represents.
///
/// Roles make it possible for various UI toolkits to expose their controls to assistive
/// technologies (ATs) with a standard interface, regardless of toolkit.
///
/// For example, a widget that acts like a conventional push button (appears unpressed; presses
/// when acted upon; invokes a certain action when pressed) can expose a `Role::PushButton` role.
///
...
pub mod tests {
	...
	#[test]
	fn test_serialization_matches_fromreor_impl() {
		let ctxt = Context::new_dbus(LE, 0);

		for role in Role::iter() {
			let encoded = to_bytes(ctxt, &role)
				.expect("A valid Role with well known representation should serialize.");

			let (zbus_role, _) = encoded.deserialize().expect("Unable to decode encoded");

			assert_eq!(role, zbus_role, "The serde `Data::deserialize`d Role and Role appear to be unequal. role: \"{role}\", deserialized role: \"{zbus_role}\"");
		}
	}
	...
	fn test_role_name() {
		...
		assert_eq!(Role::Animation.name(), "animation");
		assert_eq!(Role::Calendar.name(), "calendar");
		assert_eq!(Role::CheckBox.name(), "check box");
		assert_eq!(Role::CheckMenuItem.name(), "check menu item");
		assert_eq!(Role::ComboBox.name(), "combo box");
		assert_eq!(Role::DateEditor.name(), "date editor");
		assert_eq!(Role::DrawingArea.name(), "drawing area");

		//
		assert_eq!(Role::EditBar.name(), "edit bar");
		assert_eq!(Role::Chart.name(), "chart");

		assert_eq!(Role::Superscript.name(), "superscript");
		assert_eq!(Role::DescriptionList.name(), "description list");
		assert_eq!(Role::DescriptionTerm.name(), "description term");
		assert_eq!(Role::DescriptionValue.name(), "description value");
		assert_eq!(Role::Footnote.name(), "footnote");
		assert_eq!(Role::ContentDeletion.name(), "content deletion");
		assert_eq!(Role::ContentInsertion.name(), "content insertion");
		assert_eq!(Role::Mark.name(), "mark");
		assert_eq!(Role::Suggestion.name(), "suggestion");
		assert_eq!(Role::PushButtonMenu.name(), "push button menu");
	}

	// Testing Display impl is more a strum responsibility, but it's good to have a test here.
	#[test]
	fn test_role_display() {
		for role in Role::iter() {
			let role_str = role.to_string();
			let role_static_str = role.name();
			assert_eq!(role_str, role_static_str, "Role display impl \"{role_str}\" should match the static name: \"{role_static_str}\"");
		}
	}
}
