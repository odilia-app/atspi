...
pub enum RelationType {
  ...
  /// widget hierarchy does not map cleanly to the onscreen presentation.
	SubwindowOf,
  /// Similar to [`SubwindowOf`], but
  /// specifically used for cross-process embedding.
	Embeds,
  /// Reciprocal of [`Embeds`]. Used to
  ...
  /// frame associated with another onscreen object. Similar to [`TooltipFor`],
  ...
  /// object with the [`PopupFor`] relation usually requires
  ...
  /// object(s). See also [`DetailsFor`] and [`ErrorFor`].
	DescriptionFor,
  ...
  /// label, use [`LabelledBy`]. For an on-screen error message, use
  /// [`ErrorMessage`]. For lengthy extended descriptive information
  /// contained in an on-screen object, consider using [`RelatedDetails`] as
  ...
  /// closely reviewed.
	DescribedBy,
  /// Reciprocal of [`DetailsFor`]. Indicates
  ...
  /// information suitable for announcement only, see [`DescribedBy`].
  /// If the detailed information describes an error condition,
	/// [`ErrorFor`] should be used instead.
	Details,
  /// Reciprocal of [`Details`]. Indicates
  ...
  /// relation type should also contain [`ErrorMessage`] in their [`StateSet`].
	ErrorMessage,
