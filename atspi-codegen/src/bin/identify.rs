...
pub fn create_events_from_xml(file_name: &str) -> String {
    ...
    let iface_data = data
        ...
        .collect::<Vec<String>>()
        .join("\n");
    format!("{STRIPPER_IGNORE_START}{module_level_doc}\n{STRIPPER_IGNORE_STOP}{iface_data}")
}

// fn save_manual_doc_comments(source_file_name: &str, work_path: &Path, writable: &mut impl Write) {
//     let ignore_macros = true;
//     stripper_lib::strip_comments(work_path, source_file_name, writable, ignore_macros)
// }

// fn restore_manual_doc_comments(directory: &Path, comment_file: &str, source_file_name: &str) {
//     let ignore_macros = true;
//     let _ignore_doc_commented = false;
...
//     let source_file_path = Path::new(directory).join(source_file_name);
//     println!("Source file path: {:?}", &source_file_path);
//     let source_file = match File::open(&source_file_path) {
//         Ok(f) => f,
//         Err(e) => {
//             println!("An error occured while trying to open '{:?}': {}", &source_file_path, e);
//             return;
//         }
//     };
//     let source_reader = BufReader::new(source_file);
//     let mut source_lines = source_reader
//         .lines()
...
struct ItemLevel {
    // distance to next struct, enum, fn
    dist: u8,
    doc: Vec<String>,
}
...
/// HashMap does not (necessarilly) preerve order. Hence Vec.
    ...
    for line in buf.lines() {
        if docstate == ParseState::None {
            match line {
                ...
                _ if line.starts_with("// IgnoreBlock start") => {
        ...
        // things will get a little more comlicated if we add module level docs to inner modules in autogenerated files.
        if docstate == ParseState::ModuleLevel {
        ...
        if docstate == ParseState::ItemLevel {
            // As long as `line` starts with '//' it is still comment. a mixed block is also a block.
            if line.starts_with("//") {
                docblock.push(line.into());
            } else {
                ...
                if !line.trim().is_empty() {
                    ...
                    docvec.push((Some(line.into()), dt));
                }
            }
        }
        if docstate == ParseState::Comment {
            ...
            let docitem = Comment { doc: docblock.clone() };
            let dt = DocType::Comment(docitem);
            docvec.push((Some(line.into()), dt.clone()));
            docstate = ParseState::None;
            docblock.clear();
            counter = 0;
            continue;
        }
        if docstate == ParseState::IgnoreBlock {
            while !line.contains("// IgnoreBlock stop") {
                continue;
            }
            docstate = ParseState::None;
            continue;
        ...
        for (idx, s) in vec_of_strings.iter().enumerate() {
            if s.contains(&pat) {
                match v {
                    ...
                    DocType::Comment(Comment { ref doc }) => {
                        let i = idx + prev_inserted_lines;
                        new_vec_of_strings.splice(i..i, doc.iter().cloned());
                        prev_inserted_lines += doc.len();
                        docvec_remains.retain(|tup| *tup != (k.clone(), v.clone()));
                    }
                    _ => {
                        panic!("catchall arm.. we should not be here ");
                    }
                }
                continue;
            }
...
fn write_map_to_stdout(docvec: &Vec<(Option<String>, DocType)>) {
...
/// Load RON file, deserialize to map of docs
fn load_map(path: &Path) -> Vec<(Option<String>, DocType)> {
    let serialized =
...
fn load_saved_docvec_or_gather_new(
    ...
    if comments_path.exists() {
        let docvec = load_map(comments_path);
...
pub fn main() {
    ...
    // Load doc map if it exists, otherwise extract from source file.
